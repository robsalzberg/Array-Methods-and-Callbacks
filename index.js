...
// (a)
const newData = fifaData.filter((yearData) => {
...
function getYears(data, cb) {
  const years = [];
  cb(data).map((finalsYears) => {
    years.push(finalsYears["Year"]);
  });
  return years;
}
...
function getWinners(data, cb) {
  const winner = [];
  cb(data).filter((finals) => {
    if (finals["Home Team Goals"] > finals["Away Team Goals"]) {
      winner.push(finals["Home Team Name"]);
    } else {
      winner.push(finals["Away Team Name"]);
    }
  });
  return winner;
}
